	 ...
	 */
 	...
 	public function render($date = null, $options = array()) {
 		...
 		for ($month = 0; $month < $this->displayedMonth; $month++) {
 			...
 			$title = $table->addChild('h3', array('class' => $this->style['title']));
 			$title->content = $calendar->format('F Y');

 			$days = $table->addChild('thead')->addChild('tr');
